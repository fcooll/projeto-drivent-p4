...
describe("POST /booking", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 404 with invalid body, there's no roomId", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
    ...
    it("should respond with status 403 with invalid body, there's not vacancy", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
...
describe("PUT /booking", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 400 with invalid bookingId", async () => {
      ...
      //Teste de partição inválida booking < 1
